...
- `.interp` 指定用哪个 dynamic linker，比如 `ld-linux.so`；ld 是 Linux 的标准 Linker
- `.dynamic` 记录需要哪些动态库
...
PIC 核心原理：无论被加载到内存的任意位置，对象文件的**代码段和数据段之间的相对距离在运行时是固定常数**。通过在 `.data` 段开头建立一个 **GOT** (Global Offset Table)，保存全局变量或外部符号的实际地址。运行时 dynamic linker 更新 GOT。
...
- `GOT.plt` 是 GOT 里面专门给 PLT 使用的一块区域，当然也在 `.data` 段。
