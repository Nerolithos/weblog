...
- 注意：怎么旋转不是猜出来的。从过长的那个子树的**节点出发回到根**，对于根的孙子出发，经过父节点，到祖父 (也就是根)，需要走两次：如果两次都是同个方向，属于**顺式子树** (如上面两图)，只需朝那个方向旋转一次父节点；如果两次方向相反 (X, Y) 就是**反式子树** (如下图)，需要 **X 方向旋转父节点变成顺式子树，再 Y 方向旋转祖父节点**。
