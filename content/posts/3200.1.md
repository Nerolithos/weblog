...
运行一个项目文件：hello.cpp
```cpp
clang++ -std=c++17 -Wall -O2 hello.cpp -o hello
//clang++ #版本# #保留报错# #最佳化# #文件名# -o #编译为#
./hello
```
...
引用本地 .cpp/.h 文件：`#include "….h"` 同上

### 命名规则
- 只能用字母、数字、下划线，且不能以数字开头。
- 区分大小写：Age 和 age 是不同变量。
- 不能使用关键字（如 int、if、class 等）。
- 常见风格：camelCase（小驼峰），或 snake_case。

所有关键词全部存储在一个叫做 std 的空间中，调用时在代码顶部声明：
```cpp
