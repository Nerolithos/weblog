...
这就是 [code scheduling](https://blog.nero-lithos.com/posts/3060p3/)：编译器乱序重排指令，避开 **WAW、WAR、RAW** 依赖，如果找不到合适指令填空，就插入 NOP。非 MIPS 机中亦可用于减少 Interlock，从而减少 Stall。
...
- 控制风险来自：条件分支指令 (branch) 或 跳转指令 (`jal` or `jalr`)。

- 分支指令比例约占 **15% 到 30%**。约每 **4 到 5 条** 指令就有一条分支。意味着控制风险是**流水线性能的关键之一**。

- ⚠️ 最难预测的控制风险是 "**indirect jump for branch JALR**"。

⚠️ 控制风险的原因<u>不是 “跳转” 这个行为本身</u>，而在于直到 **EX** 这一步计算 (ALU/Comparator) 之后 (**MEM**) 才知道下个流水级真正的 IF 对象。故 `jal` 和 `jalr` **都会**产生控制风险。
...
- 如果在 EX 阶段就知道分支结果 (比如 bypass 出来)，flush 的代价变为 2 条指令： 2 × 0.2 × 0.5 = **20% CPI Penalty** (实际在 20~50%)。所以——
...
- **前跳 (forward branch) 预测为 NT**，因为编译器常把 "**then**" 部分放在顺序路径 (if, then, else...)；
  ...
  比如 `beq r3, r4, 8` → predict fall-through。
...
> 在**运行时**根据同个指令的历史行为不断学习和更新的预测方法，准确率常能到 **90%–99%**。
...
⚠️ 它在处理<u>嵌套循环</u> (**Nested Loop**) 时发生典型的 “双连错”，准确率约 **70~80%**：
...
![](https://i.postimg.cc/1Xt0JDch/jie-ping2026-03-18-20-58-52.png)这是一个**有限状态机 (finite-state machine)**。准确率约 **85~95%**
...
控制信号有：[Asel](https://blog.nero-lithos.com/posts/3060.4/)、Bsel、ALUsel (所以需要两个 Adder)
